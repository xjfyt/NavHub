...
## 3、Cloudflare 接入主站 `navigation.xjfyt.com`

### 3.1 把 DNS 移到 Cloudflare（如果还没做）
...
## 4、Cloudflare 接 MinIO 子域

如果 2.2 步已经做了公开读，`minio.xjfyt.com` 的所有 `assets/*` 请求都是 200 + 直接图片字节，可被任意中间缓存。

### 4.1 Cache 规则（已在 3.4 规则 2 覆盖）
...
### 4.3 限制 MinIO 只接受 Cloudflare 回源（可选，更安全）
...
---
- 上一篇：[02-backup-restore.md](./02-backup-restore.md)
- 下一篇：（无，这是末篇）
- 返回索引：[docs/README.md](../README.md)# Cloudflare CDN + MinIO 公开读 接入指南
