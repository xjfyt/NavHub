  ...
  return (
    <div
      ...
      style={{
        ...
        opacity: anim ? 1 : 0, transition: 'opacity 250ms ease'
      }}
    >
      <div 
      ...
      >
        ...
        <div ref={gridRef} style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
          {localItems.map((it) => {
             ...
             return (
               <div key={it.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', cursor: 'pointer' }}
                    draggable
                    onDragStart={(e) => {
                      e.dataTransfer.effectAllowed = 'move';
                      e.dataTransfer.setData('application/x-folder-item', it.id);
                      e.dataTransfer.setData('application/x-navhub-item', JSON.stringify({
                        id: it.id,
                        type: 'folder-item',
                        folderId: folder.id,
                    ...
                    onDragOver={(e) => {
                      ...
                      if (Date.now() < swapLockRef.current && lastSwapSourceRef.current === dragId && lastSwapTargetRef.current === it.id) return;
                 ...
                 <div style={{ fontSize: '13px', color: 'var(--text)', fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '80px', textAlign: 'center', opacity: dragId === it.id ? 0.01 : 1 }}>
                   {it.name}
                 </div>
               </div>
             );
