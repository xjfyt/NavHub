...
function getYearStats(now: Date) {
  ...
  const progress = Math.min(
    1,
    Math.max(0, (now.getTime() - start) / (end - start)),
  );
  return {
    year,
    elapsedDays,
...
export const YearProgressWidget = (_props: WidgetProps = {}) => {
  ...
  useEffect(() => {
    const id = window.setInterval(
      () => setStats(getYearStats(new Date())),
      60_000,
    );
  ...
  const rows: Array<{ label: string; p: number; color: string }> = [
    {
      label: "今年",
      p: s.year,
      color: "linear-gradient(90deg, #7bd88f, #6fb1ff)",
    },
    {
      label: "本月",
      p: s.month,
      color: "linear-gradient(90deg, #ff9b7b, #ffb86b)",
    },
    {
      label: "本周",
      p: s.week,
      color: "linear-gradient(90deg, #a78bfa, #60a5fa)",
    },
    {
      label: "今天",
      p: s.day,
